...
const AllPlaces = () => {
  // 🎯 1. API කෝල් කරන්නේ නැත! කෙලින්ම Redux මතකයෙන් දත්ත ලබා ගැනීම
  const { allPlaces: places, loading, error } = useSelector((state) => state.places);
  ...
  return (
    <div className="min-h-screen bg-slate-50/30 pb-24 font-sans">
      <div className="max-w-7xl mx-auto px-8 pt-8">
        ...
        {error && <div className="text-xs text-rose-400 font-bold mb-4">⚠️ {error}</div>}

        {places.length === 0 ? (
          <div className="text-center py-12 text-xs font-semibold text-slate-400">No places found inside the database.</div>
        ) : (
          /* ⚡ INSTANT RENDERING: කිසිම ප්‍රමාදයකින් තොරව සියලුම ස්ථාන පෙන්වීම */
